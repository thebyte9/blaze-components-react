...
```js
<DateTimeInput label="Date input" onChange={({ event, value }) => {}} type="date" modifier="full-width" />
```

- Pick only time

```js
<DateTimeInput label="Date input" onChange={({ event, value }) => {}} type="time" modifier="full-width" />
```
