...
CSS Custom properties enable us to make our code more efficient and allow us to work some real magic with CSS. One area where they have enormous potential is theming. At Blaze, we use Tailwind CSS, a utility class framework, for writing our styles.

We’ll look at how to use custom properties for theming and integrate them with Tailwind to maximize the reusability of our code. We won’t cover getting up and running with Tailwind — check out the [official documentation](https://tailwindcss.com/docs/installation) for that — but even if you’re new to it you might find some of these tips helpful.
...
Using custom properties for our colour themes can drastically reduce the amount of code we need to write and alleviate the maintenance burden. Let’s first take a look at how we can do this in regular CSS.

We define our custom properties as variables on the `:root` selector, making them global variables. (The body selector would serve us just as well.) Then we can use those variables in a selector in place of our colour property values in RGB format:
